	...
	<table class="pj-table b10 float_left" cellpadding="0" cellspacing="0" style="width: 79%">
		...
		foreach ($tpl['os_arr'] as $item)
		{
			...
			<tr>
						...
						foreach ($tpl['attr_arr'] as $attr)
						{
							if ($attr['id'] == $attr_parent_id)
							{
								foreach ($attr['child'] as $child)
								{
									if ($child['id'] == $attr_id)
									{
										printf('<div class="fs11"><span class="bold">%s</span>: %s</div>', pjSanitize::html($attr['name']), pjSanitize::html($child['name']));
										break;
									}
								}
							}
					...
					foreach ($a as $eid)
					{
						if (strpos($eid, ".") === FALSE)
						{
							...
							foreach ($tpl['extra_arr'] as $extra)
							{
								if ($extra['id'] == $eid)
								{
									printf('<div class="fs11"><span class="bold">Extra:</span> %s (%s)</div>', pjSanitize::html($extra['name']), pjUtil::formatCurrencySign(number_format($extra['price'], 2), $tpl['option_arr']['o_currency']));
							...
							foreach ($tpl['extra_arr'] as $extra)
							{
								if ($extra['id'] == $e_id && isset($extra['extra_items']) && !empty($extra['extra_items']))
								{
									foreach ($extra['extra_items'] as $extra_item)
									{
										...
										{
											printf('<div class="fs11"><span class="bold">Extra:</span> %s (%s)</div>', pjSanitize::html($extra_item['name']), pjUtil::formatCurrencySign(number_format($extra_item['price'], 2), $tpl['option_arr']['o_currency']));
				...
				<td class="align_right"><?php echo pjUtil::formatCurrencySign(number_format($price, 2), $tpl['option_arr']['o_currency']); ?></td>
				<td class="align_right"><?php echo pjUtil::formatCurrencySign(number_format($subtotal, 2), $tpl['option_arr']['o_currency']); ?></td>
				<td class="w60">
					<a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOrders" class="pj-table-icon-edit stock-edit" data-id="<?php echo $item['id']; ?>"></a><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOrders" class="pj-table-icon-delete stock-delete" data-id="<?php echo $item['id']; ?>"></a>
				</td>
			</tr>
			<?php
		}
