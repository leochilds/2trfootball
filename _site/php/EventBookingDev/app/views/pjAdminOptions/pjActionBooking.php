...
if (isset($tpl['status']))
{
	...
	include_once PJ_VIEWS_PATH . 'pjAdminOptions/elements/booking_menu.php';
	if (isset($tpl['arr']))
	{
		if (is_array($tpl['arr']))
		{
						...
						<td>
							...
							switch ($tpl['arr'][$i]['type'])
							{
								...
								case 'text':
									...
									break;
								case 'int':
									if(in_array($tpl['arr'][$i]['key'], array('o_deposit_payment', 'o_tax_payment'))){
										?>
										<span class="pj-form-field-custom pj-form-field-custom-before">
											<span class="pj-form-field-before"><abbr class="pj-form-field-icon-text">%</abbr></span>
											<input type="text" name="value-<?php echo $tpl['arr'][$i]['type']; ?>-<?php echo $tpl['arr'][$i]['key']; ?>" class="pj-form-field w80" value="<?php echo htmlspecialchars(stripslashes($tpl['arr'][$i]['value'])); ?>" />
										</span>
										<?php
									}else{
									...
									if(in_array($tpl['arr'][$i]['key'], array('o_booking_before_hours'))){
										?>&nbsp;<?php __('lblHours');
									}
									if(in_array($tpl['arr'][$i]['key'], array('o_require_all_within_days'))){
										?>&nbsp;<?php __('lblDays');
									}
									break;
								case 'float':
