...
if (isset($tpl['status']))
{
	...
	if (isset($tpl['arr']))
	{
		if (is_array($tpl['arr']))
		{
			...
			{
				?>
				<form id="frmOption" action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminOptions&amp;action=pjActionUpdate" method="post" class="form pj-form">
						...
						<td>
							...
							switch ($tpl['arr'][$i]['type'])
							{
								...
								case 'text':
									...
									break;
								case 'int':
									if($tpl['arr'][$i]['key'] == 'o_events_per_page')
									{
										?><input type="text" name="value-<?php echo $tpl['arr'][$i]['type']; ?>-<?php echo $tpl['arr'][$i]['key']; ?>" class="pj-form-field item-per-page w60 number" value="<?php echo htmlspecialchars(stripslashes($tpl['arr'][$i]['value'])); ?>" /><?php
									}else{
										?><input type="text" name="value-<?php echo $tpl['arr'][$i]['type']; ?>-<?php echo $tpl['arr'][$i]['key']; ?>" class="pj-form-field field-int w60 number" value="<?php echo htmlspecialchars(stripslashes($tpl['arr'][$i]['value'])); ?>" /><?php
									}
									break;
								case 'float':
