...
class pjInstaller extends pjInstallerAppController
{
					...
					if (isset($CONFIG['plugins']))
					{
						if (is_array($CONFIG['plugins']))
						{
							foreach ($CONFIG['plugins'] as $plugin)
							{
								...
								if (is_file($file))
								{
									$pdb = self::pjActionImportSQL($dbo, $file, $_SESSION[$this->defaultInstaller]['prefix'], PJ_SCRIPT_PREFIX);
									if ($pdb === false) {
										$resp['code'] = 102;
	...
	private static function pjActionGetUpdates()
	{
		$update_folder = 'app/config/updates';
	...
	public function pjActionSecureSetUpdate()
	{
		...
		if ($this->isXHR() && $this->isLoged())
		{
			...
			{
				$file_path = $update_folder . '/' . basename($_POST['name']);
				
				$response = self::pjActionExecuteSQL($dbo, $file_path);
			...
			if (isset($_POST['record']) && !empty($_POST['record']))
			{
				...
				{
					$file_path = $update_folder . '/' . basename($record);
					$response = self::pjActionExecuteSQL($dbo, $file_path);
					if ($response !== TRUE)
					{
						pjAppController::jsonResponse($response);
