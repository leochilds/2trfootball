	...
	stivaSTL.prototype = {
		...
		getCurrentLocations: function()
		{
			...
			if (navigator.geolocation) {
		    	...
		    	alert(self.options.label_geo_not_supported);
		    }
		},
		...
		bindSearchForm: function()
		{
			...
			self.$current_location.onclick = function (e) {
				if (e && e.preventDefault) {
					e.preventDefault();
				}
		...
		emptyResults: function () {
			var self = this;
			this.$search_addresses.innerHTML = self.options.label_not_found;
		...
		notFoundAddress: function () {
			var self = this;
			this.$search_addresses.innerHTML = self.options.label_address_not_found;
			this.$search_addresses.style.display = 'block';
			this.$search_directions.style.display = 'none';
			this.$search_directions.innerHTML = '';
		},
		init: function (stivaObj) {
			...
			this.$current_location = document.getElementById("stl_current_location");
			...
			self.geocoder.geocode( { 'address': self.options.default_address}, function(results, status) {
				if (status == google.maps.GeocoderStatus.OK) {
					...
					self.map = new google.maps.Map(d.getElementById("stl_store_canvas"), stlOptions);
					self.address = self.options.default_address;
					self.searchLocations();
				} else {
					self.notFoundAddress();
				}
