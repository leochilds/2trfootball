				...
				if ($id !== false && (int) $id > 0)
				{
					if (isset($_FILES['marker']) && !empty($_FILES['marker']['tmp_name']))
					{
						...
						if ($Image->getErrorCode() !== 200)
						{
							...
							if ($Image->load($_FILES['marker']))
							{
								$resp = $Image->isConvertPossible();
								if ($resp['status'] === true)
								{
									$hash = md5(uniqid(rand(), true));
									$marker_path = PJ_UPLOAD_PATH . 'markers/' . $id . '_' . $hash . '.' . $Image->getExtension();
				...
				if (isset($_FILES['marker']) && !empty($_FILES['marker']['tmp_name']))
				{
					...
					if ($Image->getErrorCode() !== 200)
					{
						...
						if ($Image->load($_FILES['marker']))
						{
							...
							{
								$hash = md5(uniqid(rand(), true));
								$marker_path = PJ_UPLOAD_PATH . 'markers/' . $_POST['id'] . '_' . $hash . '.' . $Image->getExtension();
