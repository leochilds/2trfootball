...
class Datamatrix {
	 ...
	 * @protected
	 */
	var $chset_id = array(ENC_C40 => 'C40', ENC_TXT => 'TXT', ENC_X12 =>'X12');
...
// -----------------------------------------------------------------------------
	 ...
	 * @protected
	 */
	function getGFProduct($a, $b, $log, $alog, $gf) {
		if (($a == 0) OR ($b == 0)) {
			return 0;
		}
	 ...
	 * @protected
	 */
	function getErrorCorrection($wd, $nb, $nd, $nc, $gf=256, $pp=301) {
	 ...
	 * @protected
	 */
	function get253StateCodeword($cwpad, $cwpos) {
	 ...
	 * @protected
	 */
	function get255StateCodeword($cwpad, $cwpos) {
	 ...
	 * @protected
	 */
	function lookAheadTest($data, $pos, $mode) {
	 ...
	 * @protected
	 */
	function getSwitchEncodingCodeword($mode) {
		switch ($mode) {
			case ENC_ASCII: { // ASCII character 0 to 127
				$cw = 254;
				break;
	 ...
	 * @protected
	 */
	function getMaxDataCodewords($numcw) {
	 ...
	 * @protected
	 */
	function getHighLevelEncoding($data) {
	 ...
	 * @protected
	 */
	function placeModule($marr, $nrow, $ncol, $row, $col, $chr, $bit) {
		if ($row < 0) {
			$row += $nrow;
			$col += (4 - (($nrow + 4) % 8));
		}
	 ...
	 * @protected
	 */
	function placeUtah($marr, $nrow, $ncol, $row, $col, $chr) {
	 ...
	 * @protected
	 */
	function placeCornerA($marr, $nrow, $ncol, $chr) {
	 ...
	 * @protected
	 */
	function placeCornerB($marr, $nrow, $ncol, $chr) {
	 ...
	 * @protected
	 */
	function placeCornerC($marr, $nrow, $ncol, $chr) {
		$marr = $this->placeModule($marr, $nrow, $ncol, $nrow-3, 0,       $chr, 1);
		$marr = $this->placeModule($marr, $nrow, $ncol, $nrow-2, 0,       $chr, 2);
		$marr = $this->placeModule($marr, $nrow, $ncol, $nrow-1, 0,       $chr, 3);
		$marr = $this->placeModule($marr, $nrow, $ncol, 0,       $ncol-2, $chr, 4);
	 ...
	 * @protected
	 */
	function placeCornerD($marr, $nrow, $ncol, $chr) {
		$marr = $this->placeModule($marr, $nrow, $ncol, $nrow-1, 0,       $chr, 1);
	 ...
	 * @protected
	 */
	function getPlacemetMap($nrow, $ncol) {
