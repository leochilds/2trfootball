...
class QRcode {
	 ...
	 * @since 1.0.000
	 */
	function QRcode($code, $eclevel = 'L') {
		$barcode_array = array();
	 ...
	 * @return int blacks
	 */
	 function writeFormatInformation($width, &$frame, $mask, $level) {
		$blacks = 0;
	 ...
	 * @return int b
	 */
	 function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly=false) {
		$b = 0;
		$bitMask = array();
	 ...
	 * @return array mask
	 */
	 function makeMask($width, $frame, $maskNo, $level) {
	 ...
	 */
	 function mask($width, $frame, $level) {
	 ...
	 */
	 function newInputItem($mode, $size, $data, $bstream=null) {
	 ...
	 * @return array input item
	 */
	 function encodeModeNum($inputitem, $version) {
	 ...
	 * @return array input item
	 */
	 function encodeModeAn($inputitem, $version) {
	 ...
	 * @return array input item
	 */
	 function encodeMode8($inputitem, $version) {
		$inputitem['bstream'] = array();
	 ...
	 * @return array input item
	 */
	 function encodeModeKanji($inputitem, $version) {
		$inputitem['bstream'] = array();
	 ...
	 * @return array input item
	 */
	 function encodeModeStructure($inputitem) {
		$inputitem['bstream'] = array();
	 ...
	 * @return array input item
	 */
	 function encodeBitStream($inputitem, $version) {
		$inputitem['bstream'] = array();
	 ...
	 *
	 */
	function appendNewInputItem($items, $mode, $size, $data) {
	 ...
	 * @return array items
	 */
	 function insertStructuredAppendHeader($items, $size, $index, $parity) {
	 ...
	 * @return int bits
	 */
	 function estimateBitStreamSize($items, $version) {
		$bits = 0;
		if ($version == 0) {
			$version = 1;
		}
	 ...
	 * @return int version
	 */
	 function estimateVersion($items) {
		$version = 0;
		$prev = 0;
		do {
			$prev = $version;
	 ...
	 */
	 function createBitStream($items) {
		$total = 0;
	 ...
	 * @return array bitstream
	 */
	 function appendBitstream($bitstream, $append) {
	 ...
	 * @return array bitstream
	 */
	 function appendNum($bitstream, $bits, $num) {
		if ($bits == 0) {
			return 0;
		}
	 ...
	 * @return array bitstream
	 */
	 function appendBytes($bitstream, $size, $data) {
		if ($size == 0) {
			return 0;
		}
	 ...
	 */
	 function bitstreamToByte($bstream) {
		if (is_null($bstream)) {
	 		return null;
	 	}
		$size = count($bstream);
	 ...
	 * @return array srctab
	 */
	 function qrstrset($srctab, $x, $y, $repl, $replLen=false) {
	 ...
	 */
	function getDataLength($version, $level) {
	 ...
	 */
	function getECCLength($version, $level){
	 ...
	 */
	function getMinimumVersion($size, $level) {
	 ...
	 */
	function maximumWords($mode, $version) {
		if ($mode == QR_MODE_ST) {
			return 3;
		}
		if ($version <= 9) {
	 ...
	 * @return array spec
	 */
	function getEccSpec($version, $level, $spec) {
		if (count($spec) < 5) {
			$spec = array(0, 0, 0, 0, 0);
		}
	 ...
	 * @return BCH encoded format information pattern
	 */
	function getFormatInfo($mask, $level) {
		if (($mask < 0) OR ($mask > 7)) {
			return 0;
		}
	...
	* Return block number 1
	 ...
	 * @return parity array
	 */
	 function encode_rs_char($rs, $data, $parity) {
