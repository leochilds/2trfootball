...
<div class="phpevtcal-monthly-bg">
	...
	<div id="phpevtcal_month_bar" class="phpevtcal-month-bar">
		...
		for($i = 0; $i < 13; $i++){
			...
			{
				?>
				<a class="current<?php echo $i == 12 ? ' last-short-month' : null;?>" href="javascript:void(0);"><span><?php echo date('M', strtotime($first_day_of_month));?></span></a>
				<?php
			}else{
				?>
				<a class="short-month<?php echo $i == 12 ? ' last-short-month' : null;?>" href="javascript:void(0);" rev="<?php echo date('Y', strtotime($first_day_of_month));?>" rel="<?php echo date('m', strtotime($first_day_of_month));?>"><span><?php echo date('M', strtotime($first_day_of_month));?></span></a>
				<?php
			}
	...
	<table class="phpevtcal-event-table">
		...
		{
			$day_name = date('l', strtotime($day_in_month));
			if(!empty($event_date_arr[$day_in_month]))
			{
				...
				for($j = 0; $j < $num_events; $j++)
				{
					...
					{
						?>
						<tr class="has-event<?php echo $day_name=='Sunday'? ' sunday-row': null?>">
							...
							<td>
								<label><?php echo $event_title; ?><?php echo !empty($events[$j]['category']) ? ',<span> ' . $events[$j]['category'] . '</span>':null;?></label>
				...
				?>
				<tr class="<?php echo $day_in_month == date('Y-m-d') ? 'current-date' : null; ?><?php echo $day_name=='Sunday'? ' sunday-row': null?>">
