	...
	<table class="pj-table" cellpadding="0" cellspacing="0" style="width: 100%; margin-top: 10px">
		...
		if (isset($tpl['stock_arr']))
		{
			...
			<tr class="pj-table-row-odd">
				...
				?>
				<td class="align_center"><input type="text" name="qty" value="<?php echo $tpl['os_arr']['qty']; ?>" class="pj-form-field w60" data-max="<?php echo $tpl['os_arr']['qty'] + @$tpl['stock_arr']['qty']; ?>" readonly="readonly" /></td>
				<td class="align_center"><input type="hidden" name="current_qty" value="<?php echo @$tpl['stock_arr']['qty']; ?>" /><?php echo @$tpl['stock_arr']['qty']; ?></td>
				<td><input type="hidden" name="price" value="<?php echo $tpl['os_arr']['price']; ?>" /><?php echo pjUtil::formatCurrencySign(number_format($tpl['os_arr']['price'], 2), $tpl['option_arr']['o_currency']); ?></td>
			</tr>
			<?php
					...
					foreach ($tpl['extra_arr'] as $extra)
					{
						switch ($extra['type'])
						{
							case 'single':
								...
								break;
							case 'multi':
								?><select name="extra_id[<?php echo $extra['id']; ?>]" class="pj-form-field">
									<option value="">-- Select --</option>
									<?php
									foreach ($extra['extra_items'] as $k => $item)
									{
										?><option value="<?php echo $extra['type']; ?>|<?php echo $item['price']; ?>|<?php echo $item['id']; ?>"<?php echo isset($tpl['oe_arr'][$extra['id']]) && $item['id'] == $tpl['oe_arr'][$extra['id']] ? ' selected="selected"' : NULL; ?>><?php echo pjSanitize::html($item['name']); ?> (<?php echo pjUtil::formatCurrencySign(number_format($item['price'], 2), $tpl['option_arr']['o_currency']); ?>)</option><?php
									}
									?>
									</select>
								<?php
